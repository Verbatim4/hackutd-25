...
def aggregate_hotspots(df):
    ...
    feature_df = df.groupby('cluster_id').apply(lambda x: pd.Series({
        ...
        'billing_share': (x['category'] == 'billing').mean(),
        'support_share': (x['category'] == 'support').mean(),
        'app_share': (x['category'] == 'app').mean(),
...
def build_graph(features, distance_threshold_km=2):
    ...
    coords = features[['latitude', 'longitude']].to_numpy()
...
def main():
    # === Load dataset ===
    input_file = "dfw_complaints.csv"
    print(f"Loading data from {input_file}...")
    df = pd.read_csv(input_file)

    # === Run pipeline ===
    print("Running hotspot detection and risk prediction...")
    results, model = run_pipeline(df)

    # === Save output ===
    output_file = "dfw_hotspot_risk_predictions.csv"
